...
export default class View {
	...
	async createCoinbaseView() {
		try {
			...
			const accounts = await this.addAccounts(keyArr);
	...
	async addAccounts(keyArr) {
		var accounts = [];
		for(key of keyArr) {
			const privateKey = new Buffer(key, 'hex');
			const address = ethJSUtil.privateToAddress(privateKey).toString('hex');
			this.vm.stateManager.putAccountBalance(address, '999999999999999000000000000000000', () => {});
			this.vmAccounts['0x' + address] = {
				privateKey: privateKey,
				nonce: 0
			};
			accounts.push('0x' + address);
		}
		return accounts;
	}
