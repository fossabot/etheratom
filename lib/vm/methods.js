...
export default class VmHelpers {
				...
				if(error) {
					...
					this.exeVM(coinbase, block, tx, (error, result) => {
						if(error) {
							...
							console.log(result.vm.logs);
							console.log(ethJSUtil.baToJSON(result.vm.logs));
							resolve(result);
						}
					});
				}
	...
	async inputsToArray(reactElements, methodName) {
		const args = []
		for (let i = 0; i < reactElements[methodName].childNodes.length; i++) {
			if(reactElements[methodName][i].type !== 'submit') {
				args.push(reactElements[methodName][i].value);
			}
		}
		return args;
	}
